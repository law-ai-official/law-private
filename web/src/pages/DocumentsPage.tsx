    ...
    <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto p-6" data-testid="documents-page">
      ...
      <section className="mt-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">{t("documents.list.count", { count: documents.length })}</h2>
          <button onClick={() => refreshDocs()} className="rounded-md border border-border px-2 py-1 text-xs hover:bg-muted" data-testid="docs-refresh">{t("documents.list.refresh")}</button>
        </div>
        {loading ? (
  ...
  return (
    <li className="flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm" data-testid="doc-row" data-doc-id={doc.id}>
      <button onClick={onSelect} className="flex-1 text-left">
        ...
        <span className={cn("ml-2 text-xs", STATUS_COLORS[doc.status] ?? "text-muted-foreground")} data-testid="doc-status">{t(`documents.status.${doc.status}`, { defaultValue: doc.status })}</span>
    ...
    <section className="mt-6">
      ...
      {docAnswer && (
        <div className="mt-2 rounded-md border border-border bg-muted/30 p-3 text-xs" data-testid="doc-answer">
          {docAnswer.error ? (
            <span className="text-destructive">{docAnswer.error}</span>
          ) : (
            <>
              <p>{docAnswer.answer}</p>
              {docAnswer.sources && docAnswer.sources.length > 0 && (
                <p className="mt-2 text-muted-foreground">{t("documents.query.sources", { names: docAnswer.sources.map((s) => s.name).join(", ") })}</p>
              )}
            </>
          )}
        </div>
