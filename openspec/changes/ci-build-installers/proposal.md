...
- **Tag-triggered release.** On a `v*` tag, attach both installers to a GitHub Release (via `softprops/action-gh-release`). Also expose a `workflow_dispatch` for on-demand builds that don't cut a release.
...
- **Out of scope**: Linux packaging, macOS x64/Intel (config-ready later), iOS/Android, hosted/multi-tenant backend.
