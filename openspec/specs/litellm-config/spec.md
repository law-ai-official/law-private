# litellm-config Specification

## Purpose
TBD - the litellm-config capability governs how the local services launcher and supervisor configure LiteLLM at startup: auto-generating secrets on first run, warning about immutable keys, validating required environment variables, and documenting them in the `.env` template.
