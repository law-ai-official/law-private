...
## Purpose
TBD - the desktop-supervisor capability governs the Electron main process that supervises bundled child processes (server.js, OpenConnector, LiteLLM, Postgres). This spec currently covers manifest-gated descriptor resolution; additional requirements are tracked under separate capabilities (e.g. local-services-launcher).
