# litellm-upstream-config Specification

## Purpose
TBD - governs how the bundled LiteLLM upstream endpoint and credentials are named and resolved: provider-neutral `LLM_UPSTREAM_*` environment variables take precedence over legacy `VOLCES_PLAN_*` names, with a built-in default base URL fallback, and the settings pass-through forwards both naming schemes so either resolves downstream.
